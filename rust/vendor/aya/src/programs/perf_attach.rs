...
pub(crate) fn perf_attach(
    ...
    if FEATURES.bpf_perf_link() {
        ...
        match link_fd {
            Ok(link_fd) => Ok(PerfLinkInner::Fd(FdLink::new(link_fd))),
            Err(io_error)
                if cookie.is_none()
                    && matches!(
            ...
            {
                // AstraeaOS kernels may expose BPF_LINK_CREATE while rejecting
                // PERF_EVENT links. The legacy perf ioctl attaches the same
                // loaded program and preserves enforcement semantics.
                eprintln!("AstraeaOS eBPF compatibility: using legacy perf-event attach");
