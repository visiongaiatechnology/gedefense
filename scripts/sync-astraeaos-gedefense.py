...
"""Synchronize the complete public GeDefense source tree into AstraeaOS."""
...
MANIFEST_NAME = "ASTRAEAOS-MIRROR-MANIFEST.sha256"
...
# Managed metadata (never treated as ordinary source content).
MANAGED_META_NAMES = {
    MANIFEST_NAME,
    MARKER_NAME,
...
# Ephemeral agent/tool debris only — never delete legitimate sources like .gitignore.
SCRATCH_FILE_NAMES = {
    ".manifest-regen-status.txt",
}
...
def is_scratch_name(name: str) -> bool:
    """True only for known ephemeral debris files."""
    return name in SCRATCH_FILE_NAMES


def is_excluded(relative_path: Path) -> bool:
    return (
        any(part in EXCLUDED_DIRECTORY_NAMES for part in relative_path.parts)
        or relative_path.name in MANAGED_META_NAMES
        or is_scratch_name(relative_path.name)
...
def validate_gaia_root(raw_path: str) -> tuple[Path, Path]:
    ...
    if not gaia_root.exists() or not gaia_root.is_dir():
        raise MirrorError(f"AstraeaOS root does not exist: {gaia_root}")
    if gaia_root.is_symlink():
        raise MirrorError("AstraeaOS root must not be a symlink.")

    resolved_root = gaia_root.resolve(strict=True)
    target = resolved_root / "gedefense"
    if target.exists() and target.is_symlink():
        raise MirrorError("AstraeaOS GeDefense target must not be a symlink.")
    if target.parent != resolved_root or target.name != "gedefense":
        raise MirrorError("Refusing target outside the AstraeaOS/gedefense boundary.")
...
def verify(source_root: Path, target: Path) -> None:
    ...
    for candidate in target.rglob("*"):
        ...
        if is_scratch_name(candidate.name):
            try:
                candidate.unlink()
            except OSError:
                pass
            continue
        if candidate.name in MANAGED_META_NAMES:
            continue
        if is_excluded(Path(relative.as_posix())):
            continue
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Synchronize or verify AstraeaOS/gedefense against this source tree."
    )
    parser.add_argument("command", choices=("sync", "verify"))
    parser.add_argument("astraeaos_root", help="Absolute or relative AstraeaOS repository root.")
...
def main() -> int:
    ...
    try:
        ...
        _, target = validate_gaia_root(arguments.astraeaos_root)
        # Monorepo layout: the script lives inside AstraeaOS/gedefense, so source and
        # target are the same tree. "sync" then only refreshes the managed
        # manifest + marker to match current files (no external GeDefense clone).
        if arguments.command == "sync":
            if source_root.resolve(strict=True) == target.resolve(strict=True):
                # Remove only known agent debris (never wipe .gitignore etc.).
                for name in SCRATCH_FILE_NAMES:
                    debris = target / name
                    if debris.is_file() and not debris.is_symlink():
                        try:
                            debris.unlink()
                        except OSError:
                            pass
                ...
                write_atomic(
                    target / MARKER_NAME, MARKER_VALUE.encode("utf-8"), 0o644
                )
                write_atomic(target / MANIFEST_NAME, manifest_content(files), 0o644)
            else:
    ...
    print(
        f"PASS: AstraeaOS/gedefense is byte-identical for {source_count} source files."
    )
    return 0
