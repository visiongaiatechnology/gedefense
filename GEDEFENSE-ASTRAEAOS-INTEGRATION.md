...
AstraeaOS repository/
  gedefense/
    control/ gateway/ rust/ packaging/ scripts/ integration/ ...
  integration/gedefense/
    AstraeaOS package profile and authority contract
```
...
the standalone GeDefense tree is synchronized into `AstraeaOS/gedefense` using
`scripts/sync-astraeaos-gedefense.py`. A SHA-256 manifest and byte verification
prevent silent drift. The AstraeaOS mirror is a full source mirror, not a reduced
SDK, binary drop or independently modified fork.

Only the GeDefense integration boundary inside AstraeaOS is changed by this work.
Gaia Core, KDE, image construction and the Gaia Cells lifecycle implementation
remain AstraeaOS-owned.

## 1. Evidence-based starting point

AstraeaOS currently defines this layer model:
...
- Gaia Cells is currently a design stub targeted for AstraeaOS 0.3; no cell runtime or stable IPC exists yet.
...
| Gaia integration | AstraeaOS package/profile and optional Gaia Cells adapter | OS identity, cell metadata and local desktop presentation |
...
6. AstraeaOS integration is optional. Missing Gaia components must degrade to a generic Linux host, never disable GeDefense.
...
- AstraeaOS policy and FIM profile;
...
1. **Host and Boot Evidence** — AstraeaOS identity, Secure Boot state, lockdown, TPM presence, cgroup v2 and Gaia Cells runtime presence.
...
8. **Gaia Cells Adapter — implemented** — authenticated `VGTGC1` discovery and
   ...
   without degrading host protection until AstraeaOS supplies `gaia-cellsd`.
